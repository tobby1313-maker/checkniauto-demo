...
- When using `observed_market_average_eur`, treat `eur_priced_comparable_count` as its sample size. Ads available only in CZK/PLN remain useful background but must not be implied to be part of the EUR average unless a reliable conversion was supplied.
...
- Comparable-ad links are the one customer-facing hyperlink exception. The supplied `text_research.market_comparables` list already contains only approved customer-facing records: Slovak ads first, then Czech ads, from Bazos.SK, Bazos.CZ, Autobazar.EU, or Autobazar.SK. When `source_url` is present and `verified_url` is true, use one descriptive Markdown link on that comparable's line (model/year/mileage as the link label). Do not link or individually name Polish, German, other foreign, or unsupported-portal offers; they are aggregate market background only. Do not link `web_research_findings`, technical-risk sources, VIN/history sources, the original listing source, or raw grounded-search URLs.
