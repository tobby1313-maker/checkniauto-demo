...
- Include only `text_research.market_comparables` whose `source_url` is present and `verified_url` is true. These are concrete ads and should appear in Price and Negotiation with their material differences and clickable links. Never present an unlinked estimate or omitted comparable as a car currently found on the market. If no verified linked comparable exists, do not list specific example cars and do not claim they were found; state briefly that a directly verifiable comparison needs manual checking.
