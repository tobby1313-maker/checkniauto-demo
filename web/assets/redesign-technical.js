(function () {
  ...
  const na = (lang) => lang === "en" ? "Not available" : "Nie je k dispozícii";
  ...
  function render(model) {
    payload = model;
    const lang = model.language === "en" ? "en" : "sk";
    const empty = na(lang);
    ...
    const facts = [
      [lang === "en" ? "Price" : "Cena", C.formatMoney(listing.price_eur, lang)],
      [lang === "en" ? "Mileage" : "Nájazd", mileage],
      [lang === "en" ? "Fuel" : "Palivo", listing.fuel],
      [lang === "en" ? "Transmission" : "Prevodovka", listing.transmission]
    ];
    ...
    $("[data-listing-data]").innerHTML = grid([
      [lang === "en" ? "Title" : "Názov", listing.title],
      [lang === "en" ? "Price" : "Cena", C.formatMoney(listing.price_eur, lang)],
      [lang === "en" ? "Year" : "Rok", listing.year],
      [lang === "en" ? "Mileage" : "Nájazd", mileage],
      [lang === "en" ? "Fuel" : "Palivo", listing.fuel],
      [lang === "en" ? "Engine" : "Motor", listing.engine],
      [lang === "en" ? "Transmission" : "Prevodovka", listing.transmission],
      [lang === "en" ? "Drivetrain" : "Pohon", listing.drivetrain],
      [lang === "en" ? "Location" : "Lokalita", listing.location],
      [lang === "en" ? "Source" : "Zdroj", listing.source_url]
    ], lang);
    $("[data-identity]").innerHTML = grid([
      [lang === "en" ? "Make" : "Značka", identity.make],
      [lang === "en" ? "Model" : "Model", identity.model],
      [lang === "en" ? "Generation" : "Generácia", identity.generation],
      [lang === "en" ? "Engine" : "Motor", identity.engine],
      [lang === "en" ? "Transmission" : "Prevodovka", identity.transmission],
      [lang === "en" ? "Drivetrain" : "Pohon", identity.drivetrain],
      [lang === "en" ? "Confidence" : "Spoľahlivosť", identity.confidence_label]
    ], lang);
    const identityNotes = [
      ...(identity.notes || []),
    ...
    identityNoteCard.innerHTML = identityNotes.length ? `<h3>${lang === "en" ? "Identity limitations and alternatives" : "Limity identity a možné varianty"}</h3>${list(identityNotes, empty)}` : "";
    $("[data-vin]").innerHTML = list([
      { title: "VIN", detail: listing.vin || empty },
      { title: lang === "en" ? "Format check" : "Kontrola formátu", detail: vin.format_check || empty },
      { title: lang === "en" ? "Online history" : "Online história", detail: vin.online_history || empty },
      { title: lang === "en" ? "Decoded information" : "Dekódované údaje", detail: vin.decoded_information || vin.notes || empty }
    ], empty);
    ...
    $("[data-risks]").innerHTML = risks.length ? risks.map((risk, index) => `<article class="risk-card ${String(risk.risk_level || "").toLowerCase()}"><span class="risk-index">${String(index + 1).padStart(2, "0")}</span><div><div class="risk-head"><span>${C.escapeHtml([risk.risk_level, risk.evidence_category, risk.confidence].filter(Boolean).join(" · "))}</span></div><h3>${C.escapeHtml([risk.component, risk.issue].filter(Boolean).join(" — ") || empty)}</h3><div class="risk-fields"><div><span>${lang === "en" ? "Buyer impact" : "Dopad na kupujúceho"}</span><p>${C.escapeHtml(risk.buyer_impact || empty)}</p></div><div><span>${lang === "en" ? "Vehicle-specific evidence" : "Dôkaz pre toto vozidlo"}</span><p>${C.escapeHtml(risk.specific_vehicle_evidence || empty)}</p></div><div><span>${lang === "en" ? "How to verify" : "Ako overiť"}</span><p>${C.escapeHtml(risk.verification_action || empty)}</p></div></div>${risk.low_eur !== null || risk.high_eur !== null ? `<small class="risk-estimate">${C.escapeHtml(C.formatRange(risk.low_eur, risk.high_eur, lang))}</small>` : ""}</div></article>`).join("") : `<p class="empty-state">${C.escapeHtml(empty)}</p>`;

    const marketFacts = [
      [lang === "en" ? "Asking price" : "Cena inzerátu", C.formatMoney(market.advertised_price_eur ?? listing.price_eur, lang)],
      [lang === "en" ? "Verified median" : "Overený medián", C.formatMoney(market.median_eur, lang)],
      [lang === "en" ? "Market position" : "Pozícia na trhu", market.price_view],
      [lang === "en" ? "Evidence quality" : "Kvalita podkladov", market.available ? C.confidenceLabel(market.confidence, lang) : empty]
    ];
    ...
    $("[data-market-limitations]").innerHTML = market.limitations?.length ? `<strong>${lang === "en" ? "Limitations:" : "Limity:"}</strong> ${C.escapeHtml(market.limitations.join(" "))}` : "";
    ...
    $("[data-photo-gallery]").innerHTML = photos.length ? photos.map((photo, index) => `<button class="technical-photo" type="button" data-photo-index="${index}"><img src="${C.escapeHtml(photo.url)}" alt="${C.escapeHtml((lang === "en" ? "Vehicle photo " : "Fotografia vozidla ") + (index + 1))}" loading="lazy"><span>${String(index + 1).padStart(2, "0")}</span></button>`).join("") : `<p class="empty-state">${C.escapeHtml(empty)}</p>`;
    ...
    const visionLimits = [...(vision.photo_limitations || []), ...(vision.missing_views || []).map((item) => `${lang === "en" ? "Missing view" : "Chýbajúci pohľad"}: ${item}`)];
    $("[data-vision]").innerHTML = `<h3>${lang === "en" ? "Visual findings" : "Vizuálne zistenia"}</h3>${list(observations, empty)}${visionLimits.length ? `<p class="method-note">${C.escapeHtml(visionLimits.join(" "))}</p>` : ""}`;
    C.wireLightbox(photos);

    const fallbackActions = lang === "en" ? ["Verify VIN and ownership documents.", "Arrange an independent mechanical inspection.", "Confirm service history and repair invoices."] : ["Overte VIN a doklady k vozidlu.", "Dohodnite nezávislú mechanickú prehliadku.", "Potvrďte servisnú históriu a faktúry za opravy."];
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    wireActions();
    try {
      render(await C.fetchPresentation(slug));
